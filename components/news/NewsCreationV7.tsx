...
type AutoFix = {
  ...
  label: string
  detail?: string
  // 'suggestion' = needs the reporter's decision (article unchanged)
  // 'applied'    = Berry auto-applied (metadata/formatting only) · revertible
  // 'verified'   = Berry checked and confirmed nothing to do
...
// Variants:
//   'suggestion' — Berry noticed something. Article is unchanged. Reporter decides.
//   'applied'    — Berry already changed it (metadata / invisible formatting only).
//                  Always revertible. Listed for trust, not for action.
//   'verified'   — Berry checked and confirmed there's nothing to do.
//
// Reporter-impacting decisions (grammar, spelling, headline, attribution,
// source, claims) are ALWAYS 'suggestion'. The article on disk equals what
// the reporter wrote until they tap Accept.
type SuggestionKind = 'grammar' | 'spelling' | 'headline'
const AUTO_FIXES: AutoFix[] = [
  // ── Suggestions — pending the reporter's decision ───────────────────────
  {
    id: 'grammar',
  ...
  {
    id: 'headline',
    label: 'Headline polish — needs your approval',
  ...
  // ── Auto-applied — fully revertible cleanups, never touch reporter voice ─
  {
    id: 'metadata',
  ...
  // ── Verified — Berry scanned and found nothing to flag ─────────────────
  {
    id: 'cover',
...
// Per-suggestion lifecycle inside the overlay.
//   'pending'  — Berry suggests, reporter hasn't decided yet
//   'accepted' — reporter accepted; collapsed Accepted row with Undo
//   'skipped'  — reporter dismissed; row dims (and counts toward kind-skip silence)
//   'reverted' — only for variant: 'applied' — reporter undid Berry's auto change
type SuggestionState = 'pending' | 'accepted' | 'skipped' | 'reverted'
...
function SummaryOverlay({
  ...
  // Per-row lifecycle, plus per-kind silence (3 skips of same kind → suppress
  // remaining of that kind for the session — matches the interaction spec).
  const [state, setState]   = useState<Record<string, SuggestionState>>({})
  const [silenced, setSilenced] = useState<Set<SuggestionKind>>(new Set())

  function setRowState(id: string, s: SuggestionState) {
    setState(prev => ({ ...prev, [id]: s }))
  }

  // Count skips per kind; when it hits 3, silence the kind.
  // (For demo we count distinct ids of the same kind, since each AUTO_FIXES
  // row groups multiple changes of the same kind anyway.)
  function recordSkip(fix: AutoFix) {
    setRowState(fix.id, 'skipped')
    if (['grammar', 'spelling', 'headline'].includes(fix.id)) {
      // For real per-kind memory you'd count individual changes; this demo
      // treats one skipped grouped row as one skip.
      setSilenced(prev => {
        const n = new Set(prev)
        // demo: silence the kind on first explicit skip — comment-toggle to 3
        n.add(fix.id as SuggestionKind)
        return n
      })
    }
  }

  const suggestionRows = autoFixes.filter(f => f.variant === 'suggestion')
  const appliedRows    = autoFixes.filter(f => f.variant === 'applied')
  const verifiedRows   = autoFixes.filter(f => f.variant === 'verified')

  const pendingCount = suggestionRows.filter(f => (state[f.id] ?? 'pending') === 'pending').length
  const appliedCount = appliedRows.filter(f => (state[f.id] ?? 'pending') !== 'reverted').length

  const ready = sendable && pendingCount === 0 && openIssues.length === 0

  return (
    <Backdrop onClick={onClose}>
      <div
        onClick={e => e.stopPropagation()}
        className="w-[520px] max-w-[94vw] max-h-[84vh] rounded-[20px] border border-white/70 bg-white overflow-hidden flex flex-col summary-in"
        style={{
          boxShadow:
            '0 1px 0 rgba(255,255,255,0.6) inset, ' +
            '0 30px 70px -30px rgba(15,23,42,0.35), ' +
      ...
      >
        ...
        >
          <div className="flex items-start gap-3">
            <span className={`shrink-0 inline-flex items-center justify-center size-10 rounded-[12px] text-white shadow-[0_8px_22px_-6px_rgba(124,58,237,0.45)] ${
              ready ? 'bg-gradient-to-br from-emerald-500 to-emerald-600' :
                      'bg-gradient-to-br from-[#0787ff] to-[#7c3aed]'
            }`}>
              {ready ? <Check size={18} strokeWidth={2.5} /> : <Sparkles size={18} strokeWidth={1.75} />}
            </span>
            <div className="flex-1 min-w-0">
              <span
                ...
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                {ready ? 'Ready for editorial review' : 'Almost ready'}
              </span>
              <p
              ...
              >
                {ready
                  ? 'All checks passed. Your article is unchanged.'
                  : openIssues.length > 0
                    ? `${openIssues.length} issue${openIssues.length === 1 ? '' : 's'} need${openIssues.length === 1 ? 's' : ''} your attention.`
                    : `Berry has ${pendingCount} ${pendingCount === 1 ? 'suggestion' : 'suggestions'} for you.`}
              </p>
              {!ready && (
                <p className="mt-1 text-[12.5px] text-slate-500" style={{ fontFamily: 'var(--font-inter)' }}>
                  {pendingCount > 0 && `${pendingCount} pending suggestion${pendingCount === 1 ? '' : 's'} · `}
                  {appliedCount > 0 && `${appliedCount} cleanup${appliedCount === 1 ? '' : 's'} applied (revertible)`}
                  {openIssues.length > 0 && ` · estimated fix time ${fixSeconds < 60 ? `${fixSeconds}s` : `${Math.round(fixSeconds / 60)} min`}`}
                </p>
              )}
            </div>
            <button
        ...
        {/* Body */}
        <div className="flex-1 overflow-y-auto px-6 pt-3 pb-5 flex flex-col gap-5">
          {/* Critical issues that need the reporter to do something */}
          {openIssues.length > 0 && (
          ...
          {/* Berry's suggestions — the reporter decides each one */}
          {suggestionRows.length > 0 && (
            <ExpandableSection
              defaultOpen
              label={`Berry suggests · ${suggestionRows.length}`}
              hint="Your article is unchanged"
            >
              {suggestionRows.map(f => {
                const isSilenced = ['grammar', 'spelling', 'headline'].includes(f.id) &&
                  silenced.has(f.id as SuggestionKind) &&
                  (state[f.id] ?? 'pending') === 'pending'
                if (isSilenced) {
                  return (
                    <KindSilencedRow key={f.id} fix={f} onUndo={() => {
                      setSilenced(prev => { const n = new Set(prev); n.delete(f.id as SuggestionKind); return n })
                    }} />
                  )
                }
                return (
                  <AutoFixRow
                    key={f.id}
                    fix={f}
                    state={state[f.id] ?? 'pending'}
                    onAccept={() => setRowState(f.id, 'accepted')}
                    onSkip={() => recordSkip(f)}
                    onUndo={() => setRowState(f.id, 'pending')}
                  />
                )
              })}
            </ExpandableSection>
          ...
          {/* Auto-applied cleanups — listed for trust, fully revertible */}
          {appliedRows.length > 0 && (
            <ExpandableSection
              label={`Berry already did · ${appliedRows.length}`}
              hint="Metadata and invisible cleanups · revertible"
            >
              {appliedRows.map(f => (
                <AutoFixRow
                  key={f.id}
                  ...
                  onAccept={() => {}}
                  onSkip={() => {}}
                  onUndo={() => setRowState(f.id, state[f.id] === 'reverted' ? 'pending' : 'reverted')}
                />
              ))}
            </ExpandableSection>
          )}

          {/* What Berry checked and found nothing — small print, for trust */}
          {verifiedRows.length > 0 && (
            <ExpandableSection
              label={`Looked into · ${verifiedRows.length}`}
        ...
        <div className="border-t border-slate-200 px-6 py-4 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="inline-flex items-center gap-1 text-[12.5px] font-medium text-slate-500 hover:text-slate-900 transition-colors"
              style={{ fontFamily: 'var(--font-inter)' }}
              title="See every change Berry suggested or applied this session"
            >
...
function ExpandableSection({
  label, hint, defaultOpen, children,
}: {
  label: string
  hint?: string
  defaultOpen?: boolean
    ...
    <section className="flex flex-col gap-2">
      ...
      >
        <div className="flex items-baseline gap-2">
          ...
          {hint && (
            <span className="text-[11.5px] text-slate-400" style={{ fontFamily: 'var(--font-inter)' }}>
              · {hint}
            </span>
          )}
        </div>
        <span
          className="inline-flex items-center gap-1 text-[12px] font-medium text-slate-500 group-hover:text-slate-900 transition-colors"
          style={{ fontFamily: 'var(--font-inter)' }}
        >
          {open ? 'Hide' : 'Show'}
          <ChevronRight size={12} strokeWidth={2} className={`transition-transform ${open ? 'rotate-90' : ''}`} />
        </span>
      </button>
      {open && <div className="flex flex-col berry-fixes-in">{children}</div>}
    </section>
  )
}

function KindSilencedRow({ fix, onUndo }: { fix: AutoFix; onUndo: () => void }) {
  return (
    <div className="flex items-center justify-between py-2 px-1.5 -mx-1.5 rounded-[8px] bg-slate-50/60 border-b border-slate-100 last:border-none">
      <span className="text-[12.5px] text-slate-500" style={{ fontFamily: 'var(--font-inter)' }}>
        Skipping further {fix.id} suggestions for this story.
      </span>
      <button
        type="button"
        onClick={onUndo}
...
function AutoFixRow({
  fix, state, onAccept, onSkip, onUndo,
}: {
  fix: AutoFix
  ...
  const isHeadline = fix.id === 'headline'

  // Visual map per variant
  const dotClasses =
    fix.variant === 'applied'    ? 'bg-[#0787ff]/10 text-[#0787ff]' :
    fix.variant === 'suggestion' ? 'bg-amber-50 text-amber-600' :
                                   'bg-emerald-50 text-emerald-600'

  // Tag right of the label
  let tagText: string
  let tagClass: string
  if (fix.variant === 'suggestion') {
    tagText  = state === 'accepted' ? 'Accepted'  : state === 'skipped' ? 'Skipped' : 'Pending'
    tagClass = state === 'accepted' ? 'text-emerald-600' :
               state === 'skipped'  ? 'text-slate-400' :
                                      'text-amber-600'
  } else if (fix.variant === 'applied') {
    tagText  = state === 'reverted' ? 'Reverted' : 'Applied'
    tagClass = state === 'reverted' ? 'text-slate-400' : 'text-[#0787ff]'
  } else {
    tagText  = 'OK'
    tagClass = 'text-emerald-600'
  }

  const dimmed = state === 'skipped' || state === 'reverted'

  return (
    <div className={`border-b border-slate-100 last:border-none ${dimmed ? 'opacity-60' : ''}`}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        aria-expanded={open}
      ...
      >
        <span className={`inline-flex items-center justify-center size-[18px] rounded-full ${dotClasses}`}>
          {fix.variant === 'suggestion' && state !== 'accepted'
            ? <Sparkles size={10} strokeWidth={2} />
            : <Check size={10} strokeWidth={3} />}
        </span>
        <span className="flex-1 min-w-0">
          <span className={`text-[13px] ${dimmed ? 'line-through text-slate-500' : 'text-[#0f172a]'}`} style={{ fontFamily: 'var(--font-inter)' }}>
            {fix.label}
          </span>
          {fix.detail && (
        ...
        <span
          className={`text-[10.5px] font-semibold uppercase tracking-[0.14em] ${tagClass}`}
          style={{ fontFamily: 'var(--font-inter)' }}
        >
          {tagText}
        </span>
      ...
      {open && (
        <div className="pl-[30px] pr-1.5 pt-1 pb-3 flex flex-col gap-2.5 fix-evidence-in">
          {/* Voice warning — only on headline suggestion, before the diff */}
          {isHeadline && state === 'pending' && (
            <div className="rounded-[10px] border border-amber-200 bg-amber-50/70 px-3 py-2 flex items-start gap-2">
              <span className="mt-0.5 size-[6px] rounded-full bg-amber-500 shrink-0" />
              <p className="text-[12.5px] leading-[1.5] text-amber-900" style={{ fontFamily: 'var(--font-inter)' }}>
                <span className="font-semibold">This rewrite changes the rhythm of your headline.</span> Headlines are how readers hear your byline — read it aloud before accepting.
              </p>
            </div>
          )}

          {/* Diff / evidence */}
          {fix.changes.map((c, i) => <ChangeBlock key={i} change={c} />)}

          {/* Action row — varies by variant + state */}
          {fix.variant === 'suggestion' && state === 'pending' && (
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              <button
                type="button"
                onClick={onAccept}
                className="relative inline-flex items-center justify-center min-h-[30px] px-3 rounded-[8px] overflow-hidden hover:brightness-110 transition-[filter]"
              >
                <span aria-hidden className="absolute inset-0 bg-[#0f172a] rounded-[8px]" />
                <span aria-hidden className="absolute inset-0 rounded-[8px] shadow-[inset_0_1px_0_rgba(255,255,255,0.18)]" />
                <span className="relative text-[12.5px] font-semibold text-white" style={{ fontFamily: 'var(--font-inter)' }}>
                  {isHeadline ? 'Replace my headline' : 'Accept'}
                </span>
              </button>
              <button
                type="button"
                className="inline-flex items-center justify-center min-h-[30px] px-3 rounded-[8px] bg-white border border-slate-200 text-[12.5px] font-medium text-[#0f172a] hover:bg-slate-50 transition-colors"
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                Edit & accept
              </button>
              <button
                type="button"
                onClick={onSkip}
                className="inline-flex items-center justify-center min-h-[30px] px-3 rounded-[8px] text-[12.5px] font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors"
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                Skip
              </button>
              <div className="relative">
                ...
                {whyOpen && <WhyPopover fix={fix} onClose={() => setWhyOpen(false)} />}
              </div>
            </div>
          )}
          ...
          {fix.variant === 'suggestion' && state === 'accepted' && (
            <div className="flex items-center gap-2 pt-1">
              <span className="inline-flex items-center gap-1.5 text-[12px] text-emerald-700" style={{ fontFamily: 'var(--font-inter)' }}>
                <Check size={11} strokeWidth={2.5} /> Applied to your article
              </span>
              <button
                type="button"
                onClick={onUndo}
          ...
          {fix.variant === 'suggestion' && state === 'skipped' && (
            <div className="flex items-center gap-2 pt-1">
              <span className="text-[12px] text-slate-500" style={{ fontFamily: 'var(--font-inter)' }}>
                Skipped — Berry won't bring this back.
              </span>
              <button
                type="button"
                onClick={onUndo}
          ...
          {fix.variant === 'applied' && (
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              <span className="inline-flex items-center gap-1.5 text-[12px] text-[#0787ff]" style={{ fontFamily: 'var(--font-inter)' }}>
                <Wand2 size={11} strokeWidth={2} />
                {state === 'reverted'
                  ? 'Reverted — your original is back.'
                  : 'Berry applied this for you'}
              </span>
              <button
                type="button"
                onClick={onUndo}
                className="inline-flex items-center justify-center min-h-[28px] px-2.5 rounded-[8px] text-[12px] font-medium text-slate-700 hover:bg-slate-100 transition-colors"
                style={{ fontFamily: 'var(--font-inter)' }}
              >
                {state === 'reverted' ? 'Re-apply' : 'Revert'}
              </button>
            </div>
          )}
        </div>
...
function WhyPopover({ fix, onClose }: { fix: AutoFix; onClose: () => void }) {
  const reason =
    fix.id === 'grammar'  ? 'Detected by Berry style guide rule G-12 (run-on prevention). Confidence: high.' :
    fix.id === 'spelling' ? 'No matching word in dictionary or proper-noun list. Common-typo lookalike found. Confidence: high.' :
    fix.id === 'headline' ? 'Berry compared your headline to top-performing policy headlines from your publication over the last 30 days. Suggestion is one of three candidates; you can ask for alternatives via "Edit & accept".' :
                            'Berry analyzed this against your story guidelines.'
  return (
    <div
      onMouseLeave={onClose}
      className="absolute right-0 bottom-full mb-2 z-10 w-[280px] rounded-[12px] border border-slate-200 bg-white p-3 shadow-[0_18px_40px_-18px_rgba(15,23,42,0.35),0_4px_14px_-8px_rgba(15,23,42,0.18)] why-pop-in"
    >
      <p className="text-[10.5px] font-semibold uppercase tracking-[0.16em] text-slate-400 mb-1.5" style={{ fontFamily: 'var(--font-inter)' }}>
        Why Berry suggested this
      </p>
      <p className="text-[12.5px] leading-[1.55] text-[#0f172a]" style={{ fontFamily: 'var(--font-inter)' }}>
        {reason}
      </p>
      <div className="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button className="px-2 py-1 rounded-md text-[11px] text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors" style={{ fontFamily: 'var(--font-inter)' }}>Yes</button>
          <button className="px-2 py-1 rounded-md text-[11px] text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition-colors" style={{ fontFamily: 'var(--font-inter)' }}>Wrong</button>
        </div>
      </div>
    </div>
  )
...
function V6Styles() {
  return (
    <style jsx global>{`
      ...
      .why-pop-in { animation: why-pop-in 160ms ease-out both; }
